...
/* ============================ STATE ============================ */
const REQUIRED_PROFILE_FIELDS = ["fullName","dob","phone","email","address","farmName","landArea","businessType","mainProduct","region"];
...
const state = {
  ...
  toastTimer: null,
  createForm: { productName:"", category:"", harvestDate:"", origin:"", quantity:"", unit:"kg", note:"" },
  createErrors: {},
  editForm: null,
  editErrors: {},
  batches: [
    { id:"BATCH-2026-0417", product:"Cà phê Robusta", harvestDate:"29/08/2026", quantity:"1.200 kg", status:"approved",
      origin:"Đắk Lắk", note:"N/A",
      sample:{ id:"SPL-A102", desc:"Lấy mẫu ngày 31/08/2026 · Mẫu hạt xanh, lô 1" },
      trace:{ id:"TR-88F2-A0T" }, report:"BATCH_2026_0417.pdf" },
    { id:"BATCH-2026-0418", product:"Gạo Tám thơm", harvestDate:"24/08/2026", quantity:"100.000 kg", status:"pending",
      origin:"An Giang", note:"N/A",
      sample:{ id:"SPL-B1011", desc:"Lấy mẫu ngày 27/08/2026 · Lấy mẫu tại kho chứa" } },
    { id:"BATCH-2026-0409", product:"Chè Thái Nguyên", harvestDate:"13/07/2026", quantity:"200 kg", status:"approved",
      origin:"Thái Nguyên", note:"N/A",
      sample:{ id:"SPL-C204", desc:"Lấy mẫu ngày 10/07/2026 · Mẫu lá tươi" },
      trace:{ id:"TR-51C9-K3D" }, report:"BATCH_2026_0409.pdf" },
    { id:"BATCH-2026-01199", product:"Sầu riêng Ri6", harvestDate:"22/01/2026", quantity:"100 kg", status:"rejected",
      origin:"Sóc Trăng", note:"N/A",
      sample:{ id:"SPL-A102", desc:"Lấy mẫu ngày 20/01/2026 · Mẫu lấy từ 5 trái" },
      rejectReason:"AUDITOR TỪ CHỐI LÔ HÀNG: Báo cáo kiểm nghiệm phòng lab (Laboratory Report) ghi nhận chỉ số thuốc bảo vệ thực vật, kim loại nặng vượt mức cho phép theo quy định chất lượng.",
      report:"BATCH_2026_01199.pdf" }
  ],
  profile: {
    fullName:"Nguyễn Văn A", dob:"12/01/1978", phone:"0998776544", email:"abc@gmail.com",
    address:"phường Hạc Thành, Thanh Hóa",
    farmName:"Trang trại HH", landArea:"", businessType:"Nông trại", mainProduct:"Cà phê Robusta",
    taxCode:"", region:"Thanh Hóa",
    username:"abc@gmail.com", role:"FARMER", status:"Đang hoạt động", joinDate:"05/12/2019"
  }
};
...
function statusMeta(status){
  if(status==="approved") return {label:"Đã kiểm định", cls:"badge-approved"};
  if(status==="rejected") return {label:"Bị từ chối", cls:"badge-rejected"};
...
function missingProfileFields(){
  return REQUIRED_PROFILE_FIELDS.filter(f => !state.profile[f] || String(state.profile[f]).trim()==="");
}
...
function filteredBatches(){
  ...
  if(state.search.trim()){
    ...
    list = list.filter(b => b.id.toLowerCase().includes(q) || b.product.toLowerCase().includes(q));
  }
  return list;
}
...
function render(){
  ...
  if(state.loggedOut){ app.innerHTML = renderLoggedOut(); return; }
...
function renderBatchRowsTable(list){
  ...
  }).join("") : `<tr class="empty-row"><td colspan="5">Không tìm thấy lô hàng phù hợp.</td></tr>`;
...
function renderBatchTable(list){
  const wrapId = state.page==="overview" ? "overviewTableWrap" : "batchTableWrap";
...
function fieldOrMissing(val){
  ...
  return has ? `<span class="info-value">${esc(val)}</span>` : `<span class="info-value missing">Chưa cập nhật</span>`;
}
...
function closeModal(){ state.modal=null; state.createErrors={}; state.editErrors={}; render(); }
function stop(e){ e.stopPropagation(); }

/* ---- detail modal ---- */
function openDetailModal(id){ state.modal={type:"detail", id}; render(); }
  ...
  <div class="overlay" onclick="closeModal()">
    <div class="modal" onclick="stop(event)">
      ...
      <div class="trace-box" style="margin-bottom:16px;">
        <div class="trace-qr">${ICONS.qrSmall}</div>
        <div>
          <div class="trace-label">Trace ID (chỉ xem)</div>
          <div class="trace-id">${esc(b.trace.id)}</div>
        </div>
      </div>
      <div class="section-label">Biên bản kiểm định</div>
      <div class="file-chip">${ICONS.file} ${esc(b.report)}</div>
      ` : ``}
      ...
      <div class="reject-box" style="margin-bottom:16px;">
        ...
        <div class="reject-text">${esc(b.rejectReason.replace(/^AUDITOR TỪ CHỐI LÔ HÀNG:\s*/,""))}</div>
      </div>
      <div class="section-label">Biên bản kiểm định</div>
      <div class="file-chip">${ICONS.file} ${esc(b.report)}</div>
      ` : ``}
...
function openCreateModal(){
  state.createForm = { productName:"", category:"", harvestDate:"", origin:"", quantity:"", unit:"kg", note:"" };
  state.createErrors = {};
  state.modal = {type:"create"};
  render();
}
function onCreateField(key,val){ state.createForm[key]=val; }
function renderCreateModal(){
  const f = state.createForm; const err = state.createErrors;
...
function submitCreateBatch(){
  ...
  if(!f.quantity || Number(f.quantity) <= 0) errors.quantity = "Vui lòng nhập số lượng hợp lệ";
  ...
  const displayDate = dateParts.length===3 ? `${dateParts[2]}/${dateParts[1]}/${dateParts[0]}` : f.harvestDate;
  state.batches.unshift({
    id, product:f.productName, harvestDate:displayDate,
    quantity:`${f.quantity} ${f.unit}`, status:"pending",
    origin:f.origin, note:f.note || "N/A",
...
function onEditField(key,val){ state.editForm[key]=val; }
function renderEditProfileModal(){
  const f = state.editForm; const err = state.editErrors;
  const errCls = k => err[k] ? "field error" : "field";
  const req = k => REQUIRED_PROFILE_FIELDS.includes(k) ? `<span class="req">*</span>` : "";
  return `
  <div class="overlay" onclick="closeModal()">
...
function submitEditProfile(){
  ...
  REQUIRED_PROFILE_FIELDS.forEach(k=>{
    if(!f[k] || String(f[k]).trim()==="") errors[k] = "Bắt buộc nhập thông tin này";
  });
...
function reLogin(){ state.loggedOut = false; state.page="overview"; render(); }
...
function navigate(page){
  state.page = page; state.search=""; state.filter="all"; state.modal=null; state.mobileMenuOpen=false;
  render();
}
